...
Upload -> validate -> decode frames -> run the same pose model + fall
detector used for live cameras -> emit fall events with timestamps/
confidence -> write an event clip per fall -> persist Recording + Event
rows (video_upload_id set, camera_id null) -> notify -> mark the
VideoUpload row completed, all while reporting progress over the same
realtime WebSocket channel live-camera alerts use.
...
def _process(video_upload_id: int, stored_path: str, original_filename: str):
    ...
    while True:
        ...
        if frame_index % stride == 0:
            ...
            for fall_event in fall_detector.update(people, now=video_time_seconds, classifier_scores=classifier_scores):
